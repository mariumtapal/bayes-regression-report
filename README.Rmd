---
title: "Simple Normal Regression Reports with snr():"
...
The motivation behind automating a Bayesian Simple Normal Regression is to eliminate the process of writing long code when one is first learning the topic and focus on the underlying concepts. In this project, we aim to summarize and create a big picture for your proposed analysis. This automated report does all the necessary diagnostics and tests for you to have a good understanding of your Bayesian Simple Normal Regression model. 

If your first model does not work, don't worry! You can simply can change your explanatory and response variables as needed, without having to re-write the code or copy-paste-edit it, and obtain a second model to compare with. And repeat as many times as needed. Moreover, the report will give you red flags if your model does't look great!

Our reports are also highly customizable. With our `snr()` function, you can tell the function, for example, how many iterations you want in your stan model, or the number of folds used for cross-validation checks. But if you don't want to specify, we have thoughtful defaults built-in.
...
Our `snr()` function uses a parameterized R Markdown report to generate the analysis. When you call the function, the arguments you specify get passed on to our template file (called `SNR_Template.Rmd` and can be found it in the files pane). Then, these arguments (parameters) are accessed in the template file as how you would normally use any variable to generate the customized report.

`snr()` is really just a wrapper  on `rmarkdown::render()`. We created this function so that generating the automated analysis reports is as simple as possible!

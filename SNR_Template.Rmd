...
Below is a plot of the first `r num_lines` potential regression lines proposed by the model (plotted in medium blue) alongside the mean proposed regression line (plotted in dark blue):
...
for(prediction in params$pred){
  ...
  graph <- mcmc_dens(shortcut_prediction) +
    labs(x = paste0("Most Likely Value: ", as.character(round(value_max, 2))), title = paste0("Predicted ", params$y, " when ", params$x, " = ", prediction)) +   
...
3. Can we slightly edit the posterior probability of slope section so we can't get outputs saying that the probability is 1? It's never exactly 1.
...
knitr::asis_output("## Posterior Probability Calculations")
...
1. Add in interpretations of output that are based on the output numbers in cases where they're possible. (And also add in text that describes what each graph is showing!) -- need to do interpretations for posterior probabilities / predictions.
...
5. (Idea). In the function we can add a `output_file` arg to `render()` and based on the input dataset/variables the output file will be named. For eg, knitted file would be named `SNR-bikes-temp_actual-rides.html` just for a meaningful file name